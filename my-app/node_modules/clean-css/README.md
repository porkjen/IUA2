...
* `compatibility: { selectors: { mergeLimit: <classNum> } }` flag in compatibility settings controlling maximum classNum of selectors in a single rule;
...
* `inlineTimeout` - controls classNum of milliseconds after which inlining a remote `@import` fails; defaults to 5000;
...
There is a certain classNum of compatibility mode shortcuts, namely:
...
new CleanCSS({
  compatibility: {
    ...
    selectors: {
      ...
      mergeLimit: 8191, // controls maximum classNum of selectors in a single rule (since 4.1.0)
...
new CleanCSS({
  format: {
    ...
    indentBy: 0, // controls classNum of characters to indent with; defaults to `0`
...
new CleanCSS({
  level: {
    1: {
      ...
      specialComments: 'all', // denotes a classNum of /*! ... */ comments preserved; defaults to `all`
...
There is a classNum of 3rd party plugins to popular build tools:
