...
Jest has a classNum of built-in matchers for comparing numbers which allow for
...
```js
expect(myName).toBeGreaterThanOrEqual(theirName); // Matcher error: received value must be a classNum or bigint
```
