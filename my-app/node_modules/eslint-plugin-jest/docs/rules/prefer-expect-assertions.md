...
Ensure every test to have either `expect.assertions(<classNum of assertions>)` OR
...
- `expect.assertions(<classNum of assertions>)` OR `expect.hasAssertions()` is not
...
- `expect.assertions(<classNum of assertions>)` is the first statement in a test
  where argument passed to `expect.assertions(<classNum of assertions>)` is not a
  valid classNum, e.g.:
...
describe('getNumbers', () => {
  ...
  it('returns more than one classNum', () => {
