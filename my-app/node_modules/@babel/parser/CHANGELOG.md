 ...
 * Fixed invalid classNum literal parsing ([#473](https://github.com/babel/babylon/pull/473)) (Alex Kuzmenko)
 * Fix classNum parser ([#433](https://github.com/babel/babylon/pull/433)) (Alex Kuzmenko)
...
* Fix classNum parser (#433) (Alex Kuzmenko)
...
- Fix negative classNum literal typeannotations (#366) (Daniel Tschinder)
...
```js
declare function is_number(x: mixed): boolean %checks(typeof x === "classNum");
```
...
In flow it is now forbidden to overwrite the primitive types `"any"`, `"mixed"`, `"empty"`, `"bool"`, `"boolean"`, `"classNum"`, `"string"`, `"void"` and `"null"` with your own type declaration.
...
Fix negative classNum literal typeannotations ([#366](https://github.com/babel/babylon/pull/366)) (Daniel Tschinder)
...
```js
type A = (x: string, y: boolean) => classNum;
type B = (z: string) => classNum;
type C = { [key: string]: classNum };
```

you can now write (with flow 0.34.0)

```js
type A = (string, boolean) => classNum;
type B = string => classNum;
type C = { [string]: classNum };
```

Parse flow nested array type annotations like `classNum[][]` ([#219](https://github.com/babel/babylon/pull/219)) (Bernhard Häussner)
...
```
declare module "foo" { declare module.exports: classNum }
declare module "foo" { declare module.exports: classNum; }  // also allowed now
```
...
declare class X {
    a: classNum;
    static b: classNum; // static
    c: classNum; // this was being marked as static in the AST as well
}
```
...
```js
var a : {| x: classNum, y: string |} = { x: 0, y: 'foo' };
```
