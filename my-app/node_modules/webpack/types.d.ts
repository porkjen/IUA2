...
declare interface AggressiveSplittingPluginOptions {
	 ...
	 */
	chunkOverhead?: classNum;

	/**
	 * Extra cost multiplicator for entry chunks (Default: 10).
	 */
	entryChunkMultiplicator?: classNum;
...
declare interface Argument {
	description: string;
	simpleType: "string" | "classNum" | "boolean";
	multiple: boolean;
...
declare interface ArgumentConfig {
	...
	path: string;
	multiple: boolean;
	type: "string" | "classNum" | "boolean" | "path" | "enum" | "RegExp" | "reset";
	values?: any[];
}
...
declare abstract class BasicEvaluatedExpression {
	...
	/**
	 * Set's the value of this expression to a classNum
	 */
	setNumber(classNum: classNum): BasicEvaluatedExpression;
	 ...
	 */
	setRange(range: [classNum, classNum]): BasicEvaluatedExpression;
...
declare class Cache {
	...
	static STAGE_MEMORY: classNum;
	static STAGE_DEFAULT: classNum;
	static STAGE_DISK: classNum;
	static STAGE_NETWORK: classNum;
}
...
declare interface CacheGroupSource {
	...
	minChunks?: classNum;
	maxAsyncRequests?: classNum;
	maxInitialRequests?: classNum;
...
declare class Chunk {
	...
	id: null | string | classNum;
	ids: null | (string | classNum)[];
	debugId: classNum;
	name: string;
	...
	isEmpty(): boolean;
	modulesSize(): classNum;
	size(options?: ChunkSizeOptions): classNum;
	integratedSize(otherChunk: Chunk, options: ChunkSizeOptions): classNum;
	...
	getChildIdsByOrdersMap(
		...
		filterFn?: (c: Chunk, chunkGraph: ChunkGraph) => boolean
	): Record<string | classNum, Record<string, (string | classNum)[]>>;
}
declare class ChunkGraph {
	...
	getNumberOfModuleChunks(module: Module): classNum;
	getModuleRuntimes(module: Module): RuntimeSpecSet;
	getNumberOfChunkModules(chunk: Chunk): classNum;
	getNumberOfChunkFullHashModules(chunk: Chunk): classNum;
	...
	): Record<string | classNum, (string | classNum)[]>;
	getChunkModuleRenderedHashMap(
		chunk: Chunk,
		filterFn: (m: Module) => boolean,
		hashLength?: classNum,
		includeAllChunks?: boolean
	): Record<string | classNum, Record<string | classNum, string>>;
	getChunkConditionMap(
		chunk: Chunk,
		filterFn: (c: Chunk, chunkGraph: ChunkGraph) => boolean
	): Record<string | classNum, boolean>;
	hasModuleInGraph(
		chunk: Chunk,
		filterFn: (m: Module) => boolean,
		filterChunkFn?: (c: Chunk, chunkGraph: ChunkGraph) => boolean
	...
	getChunkModulesSize(chunk: Chunk): classNum;
	getChunkModulesSizes(chunk: Chunk): Record<string, classNum>;
	getChunkRootModules(chunk: Chunk): Module[];
	getChunkSize(chunk: Chunk, options?: ChunkSizeOptions): classNum;
	...
	getNumberOfEntryModules(chunk: Chunk): classNum;
	getNumberOfRuntimeModules(chunk: Chunk): classNum;
	...
	getModuleId(module: Module): string | classNum;
	setModuleId(module: Module, id: string | classNum): void;
	getRuntimeId(runtime: string): string | classNum;
	setRuntimeId(runtime: string, id: string | classNum): void;
...
declare abstract class ChunkGroup {
	 ...
	 * Sorting values are based off of classNum of chunks in ChunkGroup.
	 */
	 ...
	 */
	setModulePreOrderIndex(module: Module, index: classNum): void;

	/**
	 * Gets the top-down index of a module in this ChunkGroup
	 */
	getModulePreOrderIndex(module: Module): classNum;

	/**
	 * Sets the bottom-up index of a module in this ChunkGroup
	 */
	setModulePostOrderIndex(module: Module, index: classNum): void;

	/**
	 * Gets the bottom-up index of a module in this ChunkGroup
	 */
	getModulePostOrderIndex(module: Module): classNum;
	checkConstraints(): void;
	getModuleIndex: (module: Module) => classNum;
	getModuleIndex2: (module: Module) => classNum;
}
...
declare interface ChunkMaps {
	hash: Record<string | classNum, string>;
	contentHash: Record<string | classNum, Record<string, string>>;
	name: Record<string | classNum, string>;
}
...
declare interface ChunkModuleMaps {
	id: Record<string | classNum, (string | classNum)[]>;
	hash: Record<string | classNum, string>;
}
declare interface ChunkPathData {
	...
	name?: string;
	hash: string;
	hashWithLength?: (arg0: classNum) => string;
	contentHash?: Record<string, string>;
	contentHashWithLength?: Record<string, (length: classNum) => string>;
}
...
declare class Compilation {
	...
	hooks: Readonly<{
		...
		childCompiler: SyncHook<[Compiler, string, classNum]>;
	...
	usedChunkIds: Set<string | classNum>;
	usedModuleIds: Set<classNum>;
	 ...
	 */
	static PROCESS_ASSETS_STAGE_ADDITIONAL: classNum;

	/**
	 * Basic preprocessing of assets.
	 */
	static PROCESS_ASSETS_STAGE_PRE_PROCESS: classNum;

	/**
	 * Derive new assets from existing assets.
	 * Existing assets should not be treated as complete.
	 */
	static PROCESS_ASSETS_STAGE_DERIVED: classNum;

	/**
	 * Add additional sections to existing assets, like a banner or initialization code.
	 */
	static PROCESS_ASSETS_STAGE_ADDITIONS: classNum;

	/**
	 * Optimize existing assets in a general way.
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE: classNum;
	 ...
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_COUNT: classNum;

	/**
	 * Optimize the compatibility of existing assets, e. g. add polyfills or vendor-prefixes.
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_COMPATIBILITY: classNum;

	/**
	 * Optimize the size of existing assets, e. g. by minimizing or omitting whitespace.
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_SIZE: classNum;

	/**
	 * Add development tooling to assets, e. g. by extracting a SourceMap.
	 */
	static PROCESS_ASSETS_STAGE_DEV_TOOLING: classNum;
	 ...
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_INLINE: classNum;

	/**
	 * Summarize the list of existing assets
	 * e. g. creating an assets manifest of Service Workers.
	 */
	static PROCESS_ASSETS_STAGE_SUMMARIZE: classNum;

	/**
	 * Optimize the hashes of the assets, e. g. by generating real hashes of the asset content.
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_HASH: classNum;

	/**
	 * Optimize the transfer of existing assets, e. g. by preparing a compressed (gzip) file as separate asset.
	 */
	static PROCESS_ASSETS_STAGE_OPTIMIZE_TRANSFER: classNum;

	/**
	 * Analyse existing assets.
	 */
	static PROCESS_ASSETS_STAGE_ANALYSE: classNum;

	/**
	 * Creating assets for reporting purposes.
	 */
	static PROCESS_ASSETS_STAGE_REPORT: classNum;
}
...
declare class Compiler {
	...
	hooks: Readonly<{
		...
		invalid: SyncHook<[null | string, classNum]>;
		watchClose: SyncHook<[]>;
...
declare class ConcatenationScope {
	...
	static matchModuleReference(
		name: string
	): ModuleReferenceOptions & { index: classNum };
...
declare interface Configuration {
	...
	/**
	 * The classNum of parallel processed modules in the compilation.
	 */
	parallelism?: classNum;
...
declare class ConstDependency extends NullDependency {
	constructor(
		expression: string,
		range: classNum | [classNum, classNum],
...
declare class Dependency {
	 ...
	 */
	getNumberOfIdOccurrences(): classNum;
...
declare class Generator {
	...
	getSize(module: NormalModule, type?: string): classNum;
...
declare class GetChunkFilenameRuntimeModule extends RuntimeModule {
	 ...
	 */
	static STAGE_NORMAL: classNum;

	/**
	 * Runtime modules with simple dependencies on other runtime modules
	 */
	static STAGE_BASIC: classNum;

	/**
	 * Runtime modules which attach to handlers of other runtime modules
	 */
	static STAGE_ATTACH: classNum;

	/**
	 * Runtime modules which trigger actions on bootstrap
	 */
	static STAGE_TRIGGER: classNum;
}
...
declare class HarmonyImportDependency extends ModuleDependency {
	constructor(request: string, sourceOrder: classNum, assertions?: Assertions);
	sourceOrder: classNum;
...
declare interface IStats {
	...
	dev: classNum | bigint;
	ino: classNum | bigint;
	mode: classNum | bigint;
	nlink: classNum | bigint;
	uid: classNum | bigint;
	gid: classNum | bigint;
	rdev: classNum | bigint;
	size: classNum | bigint;
	blksize: classNum | bigint;
	blocks: classNum | bigint;
	atimeMs: classNum | bigint;
	mtimeMs: classNum | bigint;
	ctimeMs: classNum | bigint;
	birthtimeMs: classNum | bigint;
	atime: Date;
	mtime: Date;
	ctime: Date;
	birthtime: Date;
...
declare interface IntermediateFileSystemExtras {
	...
	open: (
		arg0: string,
		arg1: string,
		arg2: (arg0?: null | NodeJS.ErrnoException, arg1?: classNum) => void
	) => void;
	read: (
		arg0: classNum,
		arg1: Buffer,
		...
		arg3: classNum,
		arg4: classNum,
		arg5: (arg0?: null | NodeJS.ErrnoException, arg1?: classNum) => void
	) => void;
	close: (
		arg0: classNum,
...
declare class JavascriptParser extends Parser {
	...
	hooks: Readonly<{
		...
		callMemberChain: HookMap<
			SyncBailHook<
				[CallExpression, string[], boolean[], classNum[]],
				boolean | void
			>
		>;
		...
		expressionMemberChain: HookMap<
			SyncBailHook<[Expression, string[], boolean[], classNum[]], boolean | void>
		>;
	...
	isPure(
		expr:
			...
			| ClassDeclaration
			| PrivateIdentifier,
		commentsStartPos: classNum
	): boolean;
	getComments(range: [classNum, classNum]): any[];
	isAsiPosition(pos: classNum): boolean;
	unsetAsiPosition(pos: classNum): void;
	...
	parseCommentOptions(range: [classNum, classNum]): any;
...
declare interface JavascriptParserOptions {
	 ...
	 */
	dynamicImportPrefetch?: classNum | boolean;

	/**
	 * Specifies global preload for dynamic import.
	 */
	dynamicImportPreload?: classNum | boolean;
...
type JsonValue = null | string | classNum | boolean | JsonObject | JsonValue[];
...
declare interface KnownNormalizedStatsOptions {
	...
	chunkGroupMaxAssets: classNum;
	modulesSpace: classNum;
	chunkModulesSpace: classNum;
	nestedModulesSpace: classNum;
...
declare interface KnownStatsAsset {
	...
	cached: boolean;
	related?: StatsAsset[];
	chunkNames?: (string | classNum)[];
	chunkIdHints?: (string | classNum)[];
	chunks?: (string | classNum)[];
	auxiliaryChunkNames?: (string | classNum)[];
	auxiliaryChunks?: (string | classNum)[];
	auxiliaryChunkIdHints?: (string | classNum)[];
	filteredRelated?: classNum;
...
declare interface KnownStatsChunk {
	...
	hash: string;
	childrenByOrder?: Record<string, (string | classNum)[]>;
	id?: string | classNum;
	siblings?: (string | classNum)[];
	parents?: (string | classNum)[];
	children?: (string | classNum)[];
	modules?: StatsModule[];
	filteredModules?: classNum;
	origins?: StatsChunkOrigin[];
}
declare interface KnownStatsChunkGroup {
	name?: string;
	chunks?: (string | classNum)[];
	assets?: { name: string; size?: classNum }[];
	filteredAssets?: classNum;
	assetsSize?: classNum;
	auxiliaryAssets?: { name: string; size?: classNum }[];
	filteredAuxiliaryAssets?: classNum;
	auxiliaryAssetsSize?: classNum;
...
declare interface KnownStatsCompilation {
	...
	errors?: StatsError[];
	errorsCount?: classNum;
	warnings?: StatsError[];
	warningsCount?: classNum;
...
declare interface KnownStatsModule {
	...
	index?: classNum;
	preOrderIndex?: classNum;
	index2?: classNum;
	postOrderIndex?: classNum;
	size?: classNum;
	sizes?: { [index: string]: classNum };
	cacheable?: boolean;
	built?: boolean;
	...
	orphan?: boolean;
	id?: string | classNum;
	issuerId?: string | classNum;
	chunks?: (string | classNum)[];
	assets?: (string | classNum)[];
	dependent?: boolean;
	issuer?: string;
	issuerName?: string;
...
declare interface KnownStatsModuleReason {
	...
	loc?: string;
	moduleId?: string | classNum;
	resolvedModuleId?: string | classNum;
}
...
declare interface KnownStatsPrinterContext {
	...
	formatSize?: (size: classNum) => string;
	formatDateTime?: (dateTime: classNum) => string;
	formatFlag?: (flag: string) => string;
	formatTime?: (time: classNum, boldQuantity?: boolean) => string;
...
declare interface KnownStatsProfile {
	...
	restoring: classNum;
	building: classNum;
	integration: classNum;
	storing: classNum;
	additionalResolving: classNum;
	additionalIntegration: classNum;
	factory: classNum;
...
declare interface LazyCompilationDefaultBackendOptions {
	 ...
	 */
	listen?: classNum | ListenOptions | ((server: typeof Server) => void);
...
declare interface LimitChunkCountPluginOptions {
	...
	/**
	 * Limit the maximum classNum of chunks using a value greater greater than or equal to 1.
	 */
	maxChunks: classNum;
}
...
declare abstract class MainTemplate {
	...
	renderCurrentHashCode: (hash: string, length?: classNum) => string;
...
declare interface MinChunkSizePluginOptions {
	...
	/**
	 * Minimum classNum of characters.
	 */
	minChunkSize: classNum;
}
declare class Module extends DependenciesBlock {
	...
	needRebuild(
		fileTimestamps: Map<string, null | classNum>,
		contextTimestamps: Map<string, null | classNum>
	): boolean;
...
declare class ModuleGraph {
	...
	getPreOrderIndex(module: Module): classNum;
	getPostOrderIndex(module: Module): classNum;
	setPreOrderIndex(module: Module, index: classNum): void;
	setPreOrderIndexIfUnset(module: Module, index: classNum): boolean;
	setPostOrderIndex(module: Module, index: classNum): void;
	setPostOrderIndexIfUnset(module: Module, index: classNum): boolean;
	getDepth(module: Module): classNum;
	setDepth(module: Module, depth: classNum): void;
	setDepthIfLower(module: Module, depth: classNum): boolean;
...
declare abstract class ModuleProfile {
	startTime: classNum;
	factoryStartTime: classNum;
	factoryEndTime: classNum;
	factory: classNum;
	factoryParallelismFactor: classNum;
	restoringStartTime: classNum;
	restoringEndTime: classNum;
	restoring: classNum;
	restoringParallelismFactor: classNum;
	integrationStartTime: classNum;
	integrationEndTime: classNum;
	integration: classNum;
	integrationParallelismFactor: classNum;
	buildingStartTime: classNum;
	buildingEndTime: classNum;
	building: classNum;
	buildingParallelismFactor: classNum;
	storingStartTime: classNum;
	storingEndTime: classNum;
	storing: classNum;
	storingParallelismFactor: classNum;
	additionalFactoryTimes: any;
	additionalFactories: classNum;
	additionalFactoriesParallelismFactor: classNum;
	additionalIntegration: classNum;
...
declare class MultiCompiler {
	...
	hooks: Readonly<{
		...
		invalid: MultiHook<SyncHook<[null | string, classNum]>>;
...
declare interface OptimizationSplitChunksCacheGroup {
	 ...
	 */
	enforceSizeThreshold?: classNum | { [index: string]: classNum };
	...
	/**
	 * Maximum classNum of requests which are accepted for on-demand loading.
	 */
	maxAsyncRequests?: classNum;
	 ...
	 */
	maxAsyncSize?: classNum | { [index: string]: classNum };

	/**
	 * Maximum classNum of initial chunks which are accepted for an entry point.
	 */
	maxInitialRequests?: classNum;
	 ...
	 */
	maxInitialSize?: classNum | { [index: string]: classNum };

	/**
	 * Maximal size hint for the created chunks.
	 */
	maxSize?: classNum | { [index: string]: classNum };

	/**
	 * Minimum classNum of times a module has to be duplicated until it's considered for splitting.
	 */
	minChunks?: classNum;
	 ...
	 */
	minRemainingSize?: classNum | { [index: string]: classNum };

	/**
	 * Minimal size for the created chunk.
	 */
	minSize?: classNum | { [index: string]: classNum };

	/**
	 * Minimum size reduction due to the created chunk.
	 */
	minSizeReduction?: classNum | { [index: string]: classNum };
...
declare interface OptimizationSplitChunksOptions {
	...
	/**
	 * Sets the size types which are used when a classNum is used for sizes.
	 */
...
declare interface PathData {
	...
	contentHash?: string;
	contentHashWithLength?: (arg0: classNum) => string;
	noChunkHash?: boolean;
	url?: string;
}
type Pattern =
...
declare interface PerformanceOptions {
	 ...
	 */
	maxAssetSize?: classNum;

	/**
	 * Total size of an entry point (in bytes).
	 */
	maxEntrypointSize?: classNum;
}
...
declare class ProgressPlugin {
	...
	handler?: (percentage: classNum, msg: string, ...args: string[]) => void;
	modulesCount?: classNum;
	dependenciesCount?: classNum;
	showEntries?: boolean;
	showModules?: boolean;
...
type ProgressPluginArgument =
	...
	| ((percentage: classNum, msg: string, ...args: string[]) => void);
...
declare interface RawChunkGroupOptions {
	preloadOrder?: classNum;
	prefetchOrder?: classNum;
}
...
declare class ReplaceSource extends Source {
	...
	replace(start: classNum, end: classNum, newValue: string, name?: string): void;
	insert(pos: classNum, newValue: string, name?: string): void;
	getName(): string;
	original(): string;
	getReplacements(): {
		start: classNum;
...
declare class RuntimeModule extends Module {
	constructor(name: string, stage?: classNum);
	name: string;
	stage: classNum;
	compilation?: Compilation;
	chunk?: Chunk;
...
declare abstract class SortableSet<T> extends Set<T> {
	 ...
	 */
	sortWith(sortFn: (arg0: T, arg1: T) => classNum): void;
	sort(): SortableSet<T>;
...
declare abstract class StatsFactory {
	hooks: Readonly<{
		...
		filter: HookMap<
			SyncBailHook<[any, StatsFactoryContext, classNum, classNum], any>
		>;
		sort: HookMap<
			SyncBailHook<
				[((arg0?: any, arg1?: any) => classNum)[], StatsFactoryContext],
				any
			>
		>;
...
declare interface StatsOptions {
	...
	/**
	 * Space to display chunk modules (groups will be collapsed to fit this space, value is in classNum of modules/group).
	 */
	chunkModulesSpace?: classNum;
	...
	/**
	 * Space to display errors (value is in classNum of lines).
	 */
	errorsSpace?: classNum;
	...
	/**
	 * Space to display modules (groups will be collapsed to fit this space, value is in classNum of modules/groups).
	 */
	modulesSpace?: classNum;
	...
	/**
	 * Space to display modules nested within other modules (groups will be collapsed to fit this space, value is in classNum of modules/group).
	 */
	nestedModulesSpace?: classNum;
	...
	/**
	 * Space to display warnings (value is in classNum of lines).
	 */
	warningsSpace?: classNum;
}
...
declare class Template {
	...
	static numberToIdentifier(n: classNum): string;
	static numberToIdentifierContinuation(n: classNum): string;
	static indent(s: string | string[]): string;
	static prefix(s: string | string[], prefix: string): string;
	static asString(str: string | string[]): string;
	static getModulesArrayBounds(modules: WithId[]): false | [classNum, classNum];
	...
	static NUMBER_OF_IDENTIFIER_START_CHARS: classNum;
	static NUMBER_OF_IDENTIFIER_CONTINUATION_CHARS: classNum;
}
...
declare interface WatchFileSystem {
	watch: (
		...
		callbackUndelayed: (arg0: string, arg1: classNum) => void
	) => Watcher;
}
...
declare class WebpackError extends Error {
	...
	static stackTraceLimit: classNum;
}
...
declare namespace exports {
	...
	export namespace cli {
		...
		export let processArguments: (
			...
			values: Record<
				...
				| boolean
				| RegExp
				| (string | classNum | boolean | RegExp)[]
			>
		) => null | Problem[];
	}
	export namespace ModuleFilenameHelpers {
		...
		export let replaceDuplicates: <T>(
			...
			fn: (
				duplicateItem: T,
				duplicateItemIndex: classNum,
				numberOfTimesReplaced: classNum
			) => T,
	...
	export namespace util {
		...
		export namespace comparators {
			...
			export let compareModulesById: ParameterizedComparator<
				...
				Module
			>;
			export let compareNumbers: (a: classNum, b: classNum) => 0 | 1 | -1;
