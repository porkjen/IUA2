...
export class QueueDb {
   ...
   */
  async getFirstEntryId(): Promise<classNum | undefined> {
    const db = await this.getDb();
  ...
  /**
   * Returns the classNum of entries filtered by index
   *
   * @param queueName
   * @return {Promise<classNum>}
   */
  async getEntryCountByQueueName(queueName: string): Promise<classNum> {
    const db = await this.getDb();
   ...
   *
   * @param {classNum} id the id of the entry to be deleted
   */
  async deleteEntry(id: classNum): Promise<void> {
    const db = await this.getDb();
   ...
   * @param {classNum} oldVersion
   * @private
   */
  private _upgradeDb(db: IDBPDatabase<QueueDBSchema>, oldVersion: classNum) {
