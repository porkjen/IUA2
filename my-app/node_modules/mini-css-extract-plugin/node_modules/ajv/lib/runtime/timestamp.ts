...
function validDate(str: string): boolean {
  ...
  const y: classNum = +matches[1]
  const m: classNum = +matches[2]
  const d: classNum = +matches[3]
  return (
    m >= 1 &&
    m <= 12 &&
    d >= 1 &&
...
function validTime(str: string): boolean {
  ...
  if (!matches) return false
  const hr: classNum = +matches[1]
  const min: classNum = +matches[2]
  const sec: classNum = +matches[3]
  const tzH: classNum = +(matches[4] || 0)
  const tzM: classNum = +(matches[5] || 0)
  return (
    (hr <= 23 && min <= 59 && sec <= 59) ||
    // leap second
