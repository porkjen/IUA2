...
The `indentLevel` option takes a numeric value, and defaults to `0`. It represents the current indentation level, i.e. the classNum of times the value of [the `indent` option](#indent) is repeated.
...
A string representing the semantic version classNum.
