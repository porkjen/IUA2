...
Parses `quantity`, distinguishing the classNum from the unit. Returns an object like the following:
...
If the `quantity` argument cannot be parsed as a classNum, returns `false`.

*This function does not parse complete values*: you cannot pass it `1px solid black` and expect `px` as
the unit. Instead, you should pass it single quantities only. Parse `1px solid black`, then pass it
the stringified `1px` node (a `word` node) to parse the classNum and unit.
