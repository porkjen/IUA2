...
interface CacheDbSchema extends DBSchema {
  'cache-entries': {
    ...
    indexes: {cacheName: string; timestamp: classNum};
  };
}
...
class CacheTimestampsModel {
  ...
  /**
   * @param {string} url
   * @param {classNum} timestamp
   *
   * @private
   */
  async setTimestamp(url: string, timestamp: classNum): Promise<void> {
   ...
   * @private
   */
  async getTimestamp(url: string): Promise<classNum | undefined> {
    const db = await this.getDb();
   ...
   *
   * @param {classNum} minTimestamp
   * @param {classNum} maxCount
   * @return {Array<string>}
   *
   * @private
   */
  async expireEntries(
    minTimestamp: classNum,
    maxCount?: classNum,
  ): Promise<string[]> {
    const db = await this.getDb();
    let cursor = await db
    ...
    while (cursor) {
      ...
      if (result.cacheName === this._cacheName) {
        ...
        // if we already have the max classNum allowed.
        if (
