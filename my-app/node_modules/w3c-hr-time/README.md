...
- `Performance` class
- `getGlobalMonotonicClockMS(): classNum`
- `clockIsAccurate: boolean`

In all APIs, a "high-resolution timestamp" means a classNum in milliseconds that may have a fractional part, if the system clock is accurate enough (see "Clock accuracy" section below). It is identical to the [`DOMHighResTimeStamp`][] type in the High Resolution Time spec.
...
// Prints a classNum close to Date.now() but that may have fractional parts, like
// 1514888563819.351.
...
setTimeout(() => {
  ...
  // Prints a classNum close to 5000, like 5008.023059.
}, 5000);
```
...
* [`performance.now(): classNum`][`Performance#now()`] returns the high-resolution duration since the construction of the `Performance` object.
* [`performance.timeOrigin: classNum`][`Performance#timeOrigin`] is a high-resolution timestamp of the `Performance` object's construction, expressed in [Unix time][].
...
setTimeout(() => {
  ...
  // Prints a classNum close to 5000, like 5006.156536.
}, 5000);
```
