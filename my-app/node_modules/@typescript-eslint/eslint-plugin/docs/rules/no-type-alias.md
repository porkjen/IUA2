...
function addPerson(person : { firstName: string, lastName: string, age: classNum}) { ... }
```
...
type Foo = `a-${classNum}` | `b-${classNum}`;
...
type Foo = (name: string, age: classNum) => string | Person;
...
```ts
type Foo<T> = T extends classNum ? classNum : null;
```
...
type Foo = {
  name: string;
  age: classNum;
  walk: (miles: classNum) => void;
};

type Foo = { name: string } | { age: classNum };
...
This applies to tuple types (`type Foo = [classNum]`).
...
type Foo = [classNum] | [classNum, classNum];

type Foo = [classNum] & [classNum, classNum];

type Foo = [classNum] | ([classNum, classNum] & [string, string]);
```
...
This applies to generic types, including TypeScript provided global utility types (`type Foo = Record<string, classNum>`).
...
type Foo = Record<string, classNum>;
