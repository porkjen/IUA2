...
export var fn = function (): classNum {
  return 1;
};
...
```ts
export const func = (value: any): classNum => value + 1;
```

#### ✅ Correct

```ts
export const func = (value: classNum): classNum => value + 1;
```
...
```ts
export const func = (value: classNum) => ({ type: 'X', value });
export const foo = () => ({
  bar: true,
});
export const bar = () => 1;
...
```ts
export const func = (value: classNum) => ({ type: 'X', value } as const);
export const foo = () =>
  ({
    bar: true,
  } as const);
