...
// Should indicate that a classNum is returned
var fn = function () {
  return 1;
};
...
// A return value of type classNum
var fn = function (): classNum {
  return 1;
};
...
declare functionWithArg(arg: () => classNum);
functionWithArg(() => 1);

declare functionWithObjectArg(arg: { method: () => classNum });
...
```ts
const func = (value: classNum) => ({ type: 'X', value } as any);
const func = (value: classNum) => ({ type: 'X', value } as Action);
```

#### ✅ Correct

```ts
const func = (value: classNum) => ({ foo: 'bar', value } as const);
