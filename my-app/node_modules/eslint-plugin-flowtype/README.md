...
type X = (string | classNum)[]
// Message: Use "Array<string | classNum>", not "(string | classNum)[]"
...
// Message: Use "Array<{ foo: string, bar: classNum }>", not "{ foo: string, bar: classNum }[]"
...
// Message: Use "Array<Promise<{ foo: string, bar: classNum }>>", not "Promise<{ foo: string, bar: classNum }>[]"
...
// Options: ["never"]
type X = { [key: string]: classNum, foo: string, }
// Message: Unexpected trailing delimiter
...
// Options: ["never"]
type X = { foo: string, [key: string]: classNum, }
// Message: Unexpected trailing delimiter
...
// classNum or string
type Y = string | classNum;

// resting + sleep
type snooze = "dreaming" | "";

type Props = {
  accountBalance: string | classNum,
  accountNumber: string | classNum,
};
...
```js
type f = { a: classNum, b: string, a: classNum }
// Message: Duplicate property.

type f = { a: classNum, b: string, a: string }
// Message: Duplicate property.
...
```js
type FooType = { a: classNum, b: string, c: classNum }

// Settings: {"flowtype":{"onlyFilesWithFlowAnnotation":true}}
type FooType = { a: classNum, b: string, a: classNum }
...
type f = { a: classNum, b: string, c: classNum }
...
```js
type Action
  = { type: 'FOO', doFoo: (_: classNum) => void }
  | { type: 'BAR', doBar: (_: string) => void };

type State = { foo: classNum, bar: string };
...
function foo(bar: { n: classNum } | { s: string }) {}
// Message: All union types must be declared with named type alias.

function foo(bar: { n: classNum } & { s: string }) {}
// Message: All intersection types must be declared with named type alias.

const foo: { n: classNum } & { s: string } = { n: 0, s: "" };
// Message: All intersection types must be declared with named type alias.

type Foo = { bar: { n: classNum } & { s: string } };
// Message: All intersection types must be declared with named type alias.
...
type Foo = { disjoint: "A", n: classNum } | { disjoint: "B", s: string };

type Foo = { n: classNum } & { s: string };

type Bar = { n: classNum } & { s: string }; function foo(bar: Bar) {}
...
type fn = (a: string, b: classNum) => classNum;
const f: fn = (a, b) => {}
...
type Props = {| foo: string |} | {| bar: classNum |}; class Foo extends Component<Props> { }
// Message: Props must be $ReadOnly
...
// Options: ["always",{"annotateUndefined":"always"}]
function* doThing(): Generator<classNum, void, void> { yield 2; }
...
class Test { foo = (): classNum => { return 42; } }

class Test { foo = (): classNum => 42; }

async (foo): Promise<classNum> => { return 3; }
...
// Options: ["always",{"excludeMatching":["bar"]}]
function foo(): classNum { return 42; }
function bar() { return 42; }
...
// Options: ["always",{"annotateUndefined":"always"}]
function * foo(): Iterable<classNum> { yield 2; }

// Options: ["always",{"annotateUndefined":"always"}]
async function * foo(): AsyncIterable<classNum> { yield 2; }
```
...
const foo = 3;
opaque type Foo = classNum;
// Message: All type declaration must be at the top of the file, after any import declarations.

const foo = 3;
export type Foo = classNum;
// Message: All type declaration must be at the top of the file, after any import declarations.

const foo = 3;
export opaque type Foo = classNum;
// Message: All type declaration must be at the top of the file, after any import declarations.

const foo = 3;
type Foo = classNum | string;
// Message: All type declaration must be at the top of the file, after any import declarations.
...
var foo : string = "bar", bar : classNum = 1

// Options: [{"excludeVariableMatch":"^_"}]
var _foo = "bar", bar : classNum = 1
...
// Options: ["asc"]
type FooType = { 1: classNum, 10: classNum, 2: boolean }
...
// Message: Expected type annotations to be in ascending order. "b" must be before "c".
        ...
        type FooType = {
          a: $ReadOnlyArray<classNum>,
          c: $ReadOnlyMap<string, classNum>,
          b: Map<string, Array<Map<string, classNum>>>,
        }
...
// Message: Expected type annotations to be in ascending order. "d" must be before "e".
        ...
        type FooType = {
          ...BPreservesSpreadOrderAndTypeArgs<string, classNum>,
          ...APreservesSpreadOrderAndTypeArgs<classNum>,
          c: string,
          b: classNum,
        }
...
type FooType = { a: classNum, b: classNum, c: (boolean | classNum) }

type FooType = { a: string, b: foo, C: classNum }

type FooType = { 1: classNum, 2: boolean, 10: classNum }

// Options: ["desc"]
type FooType = { c: classNum, b: classNum, a: classNum }

// Options: ["desc"]
type FooType = { C: classNum, b: string, a: {} }

// Options: ["desc"]
type FooType = { 10: classNum, 2: classNum, 1: boolean }

// Settings: {"flowtype":{"onlyFilesWithFlowAnnotation":true}}
type FooType = { b: classNum, a: classNum }

type FooType = { a: string, b(): classNum, c: boolean }
...
* `object` - Object types (`{ a: string }`, `{ [key: string]: classNum }`)
...
// Message: Expected union members to be in ascending order. "classNum[]" should be before "string[]".
// Message: Expected union members to be in ascending order. "B" should be before "classNum[]".
// Message: Expected union members to be in ascending order. "A" should be before "B".
// Message: Expected union members to be in ascending order. "string" should be before "A".
...
(foo:{ a: string, b: classNum }) => {}
// Message: There must be a space after "foo" parameter type annotation colon.

({ a, b } :{ a: string, b: classNum }) => {}
...
type TArrayPredicate = (el: T, i?:classNum) => boolean
// Message: There must be a space after "i" parameter type annotation colon.
...
declare class X { static foo:classNum }
...
function x({ a, b }: { a: string, b: classNum }) {}
...
declare class Foo { static bar :classNum; }
...
type foo = {test: classNum}; type bar = {...foo}
// Message: Use $Exact to make type spreading safe.
```
...
type foo = {test: classNum}; type bar = {...$Exact<foo>}
```
...
type X = {x: string}|{y: classNum};
// Message: There must be a space before union type annotation separator
// Message: There must be a space after union type annotation separator

type X = string | classNum |boolean;
// Message: There must be a space after union type annotation separator
