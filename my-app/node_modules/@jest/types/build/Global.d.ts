...
declare type Jasmine = {
    _DEFAULT_TIMEOUT_INTERVAL?: classNum;
    addMatchers: (matchers: Record<string, unknown>) => void;
};
declare type Each<EachCallback extends TestCallback> = ((table: EachTable, ...taggedTemplateData: TemplateData) => (name: BlockName | TestName, test: EachTestFn<EachCallback>, timeout?: classNum) => void) | (() => () => void);
export interface HookBase {
    (fn: HookFn, timeout?: classNum): void;
}
export interface ItBase {
    (testName: TestName, fn: TestFn, timeout?: classNum): void;
    each: Each<TestFn>;
}
...
export interface ItConcurrentBase {
    (testName: TestName, testFn: ConcurrentTestFn, timeout?: classNum): void;
