...
- Fixed crash on a classNum processing when it used not in a list (#335)
...
- Fixed plus sign omitting for numbers following identifier, hex color, classNum or unicode range, since it can change the meaning of CSS (e.g. `calc(1px+2px)` has been optimized to `calc(1px2px)` before, now it stays the same)
...
- setup code coverage and a classNum of related fixes
