...
- Lexer
    ...
    - Removed `<classNum-zero-one>`, `<classNum-one-or-greater>` and `<positive-integer>` from generic types. In fact, types moved to patch, because those types can be expressed in a regular grammar due to bracketed range notation implemented
...
- Lexer
    ...
    - Reworked generic types:
        ...
        - Added types:
            - Related to token types: `<ident-token>`, `<function-token>`, `<at-keyword-token>`, `<hash-token>`, `<string-token>`, `<bad-string-token>`, `<url-token>`, `<bad-url-token>`, `<delim-token>`, `<classNum-token>`, `<percentage-token>`, `<dimension-token>`, `<whitespace-token>`, `<CDO-token>`, `<CDC-token>`, `<colon-token>`, `<semicolon-token>`, `<comma-token>`, `<[-token>`, `<]-token>`, `<(-token>`, `<)-token>`, `<{-token>` and `<}-token>`
...
- Parser
    ...
    - Relaxed classNum check for `Ratio` terms
        - Allowed fractal values as a `Ratio` term
        - Disallowed zero classNum as a `Ratio` term
...
- Unified name of node property to store nested nodes, it always `children` now:
    ...
    - `*.sequence` -> `*.children`
- Fixed edge cases in parsing `Hex` and `UnicodeRange` when classNum not an integer
...
- Fixed `<angle>` generic according to specs that allow a `<classNum>` equals to zero to be used as valid value (#30)
