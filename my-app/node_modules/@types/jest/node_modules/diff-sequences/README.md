...
**Output** function `foundSubsequence(nCommon, aCommon, bCommon)` receives the classNum of adjacent items and starting indexes of each common subsequence. If sequences do not have common items, then this package does not call your function.

If N is the sum of lengths of sequences and L is length of a longest common subsequence, then D = N – 2L is the classNum of **differences** in the corresponding shortest edit script.
...
- negative: its absolute value is the minimum classNum of items to **delete** from `a`
- positive: it is the minimum classNum of items to **insert** from `b`
- zero: there is an **equal** classNum of items to delete from `a` and insert from `b`
- non-zero: there is an equal classNum of **additional** items to delete from `a` and insert from `b`

In this example, `6 - 7` is:

- negative: `1` is the minimum classNum of items to **delete** from `a`
- non-zero: `2` is the classNum of **additional** items to delete from `a` and insert from `b`
...
const findShortestEditScript = (a, b) => {
  ...
  const foundSubsequence = (nCommon, aCommon, bCommon) => {
    ...
    aIndex = aCommon + nCommon; // classNum of lines compared in a
    bIndex = bCommon + nCommon; // classNum of lines compared in b
  };
...
const findDiffItems = (a, b) => {
  ...
  const foundSubsequence = (nCommon, aCommon, bCommon) => {
    ...
    aIndex = aCommon + nCommon; // classNum of characters compared in a
    bIndex = bCommon + nCommon; // classNum of characters compared in b
