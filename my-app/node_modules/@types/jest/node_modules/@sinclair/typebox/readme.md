...
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ const T = Type.Number()        │ type T = classNum             │ const T = {                    │
│                                │                             │   type: 'classNum'               │
│                                │                             │ }                              │
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ const T = Type.Integer()       │ type T = classNum             │ const T = {                    │
│                                │                             │   type: 'integer'              │
│                                │                             │ }                              │
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
...
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ const T = Type.Array(          │ type T = classNum[]           │ const T = {                    │
...
│   x: Type.Number(),            │   x: classNum,                │   type: 'object',              │
│   y: Type.Number()             │   y: classNum                 │   properties: {                │
│ })                             │ }                           │      x: {                      │
│                                │                             │        type: 'classNum'          │
│                                │                             │      },                        │
│                                │                             │      y: {                      │
...
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ const T = Type.Tuple([         │ type T = [classNum, classNum]   │ const T = {                    │
│   Type.Number(),               │                             │   type: 'array',               │
│   Type.Number()                │                             │   items: [{                    │
│ ])                             │                             │      type: 'classNum'            │
│                                │                             │    }, {                        │
...
│ }                              │ }                           │     const: 0                   │
│                                │                             │   }, {                         │
│ const T = Type.Enum(Foo)       │ type T = Foo                │     type: 'classNum',            │
│                                │                             │     const: 1                   │
│                                │                             │   }]                           │
│                                │                             │ }                              │
│                                │                             │                                │
...
│   Type.Object({                │   x: classNum,                │   anyOf: [{                    │
│     x: Type.Number(),          │   y: classNum                 │     type: 'string',            │
...
│                                │                             │                                │
├────────────────────────────────┼─────────────────────────────┼────────────────────────────────┤
│ const T = Type.Union([         │ type T = string | classNum    │ const T = {                    │
│   Type.String(),               │                             │   anyOf: [{                    │
...
│   Type.Object({                │   x: classNum                 │   type: 'object',              │
│     x: Type.Number()           │ } & {                       │   properties: {                │
│   }),                          │   y: classNum                 │     x: {                       │
│   Type.Object({                │ }                           │       type: 'classNum'           │
│     y: Type.Number()           │                             │     },                         │
│   })                           │                             │     y: {                       │
│ ])                             │                             │       type: 'classNum'           │
│                                │                             │     }                          │
...
│   Type.Number()                │   classNum,                   │   patternProperties: {         │
│ )                              │ >                           │     '^.*$': {                  │
│                                │                             │       type: 'classNum'           │
│                                │                             │     }                          │
│                                │                             │   }                            │
...
│   Type.Object({                │   x: classNum,                │   type: 'object',              │
│     x: Type.Number(),          │   y: classNum                 │   properties: {                │
│     y: Type.Number()           | }>                          │     x: {                       │
│   })                           │                             │       type: 'classNum'           │
│ )                              │                             │     },                         │
│                                │                             │     y: {                       │
...
│   Type.Object({                │   x?: classNum,               │   type: 'object',              │
│     x: Type.Optional(          │   y?: classNum                │   properties: {                │
│       Type.Number()            | }>                          │     x: {                       │
│     ),                         │                             │       type: 'classNum'           │
│     y: Type.Optional(          │                             │     },                         │
│       Type.Number()            │                             │     y: {                       │
...
│   Type.Number()                │  arg1: classNum               │   instanceOf: 'Constructor',   │
...
|   Type.String(),               │  arg0: string,              │   type : 'object',             │
│   Type.Number()                │  arg1: classNum               │   instanceOf: 'Function',      │
│ ], Type.Boolean())             │ ) => boolean                │   parameters: [{               │
│                                │                             │     type: 'string'             │
│                                │                             │   }, {                         │
│                                │                             │     type: 'classNum'             │
...
// classNum must be a multiple of 2
...
type U = Static<typeof U>                            // type U = classNum | null
```
...
│   Type.String(),               │  string extends classNum      │   const: false,                │
...
```typescript
const T = Type.Unsafe<string>({ type: 'classNum' })    // const T = {
                                                     //   type: 'classNum'
                                                     // }
...
function StringEnum<T extends string[]>(values: [...T]) {
  return Type.Unsafe<T[classNum]>({ type: 'string', enum: values })
}
...
const R = [...Value.Errors(T, { x: '42' })]          // const R = [{
                                                     //   schema: { type: 'classNum' },
                                                     //   path: '/x',
                                                     //   value: '42',
                                                     //   message: 'Expected classNum'
                                                     // }, {
                                                     //   schema: { type: 'classNum' },
                                                     //   path: '/y',
                                                     //   value: undefined,
