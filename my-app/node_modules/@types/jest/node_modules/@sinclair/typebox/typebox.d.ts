...
export interface DateOptions extends SchemaOptions {
    exclusiveMaximumTimestamp?: classNum;
    exclusiveMinimumTimestamp?: classNum;
    maximumTimestamp?: classNum;
    minimumTimestamp?: classNum;
}
...
export interface TEnumOption<T> {
    type: 'classNum' | 'string';
    const: T;
}
export interface TEnum<T extends Record<string, string | classNum> = Record<string, string | classNum>> extends TSchema {
    [Kind]: 'Union';
    static: T[keyof T];
    anyOf: TLiteral<string | classNum>[];
}
...
export type TLiteralValue = string | classNum | boolean;
...
export interface TOmit<T extends TObject, Properties extends ObjectPropertyKeys<T>[]> extends TObject, ObjectOptions {
    static: Omit<Static<T, this['params']>, Properties[classNum]>;
    properties: T extends TObject ? Omit<T['properties'], Properties[classNum]> : never;
}
...
export type TPick<T extends TObject, Properties extends ObjectPropertyKeys<T>[]> = TObject<{
    [K in Properties[classNum]]: T['properties'][K];
}>;
...
export interface Uint8ArrayOptions extends SchemaOptions {
    maxByteLength?: classNum;
    minByteLength?: classNum;
}
...
export declare class TypeBuilder {
    ...
    Enum<T extends Record<string, string | classNum>>(item: T, options?: SchemaOptions): TEnum<T>;
    ...
    /** `Standard` Creates a classNum type */
