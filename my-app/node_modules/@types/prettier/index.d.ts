...
// array, otherwise it's going to just be classNum.
type IndexProperties<T extends { length: classNum }> = IsTuple<T> extends true
...
type IndexValue<T, P> = T extends any[] ? (P extends classNum ? T[P] : never) : P extends keyof T ? T[P] : never;
...
type CallCallback<T, U> = (path: AstPath<T>, index: classNum, value: any) => U;
type EachCallback<T> = (path: AstPath<ArrayElement<T>>, index: classNum, value: any) => void;
type MapCallback<T, U> = (path: AstPath<ArrayElement<T>>, index: classNum, value: any) => U;

// https://github.com/prettier/prettier/blob/main/src/common/ast-path.js
export class AstPath<T = any> {
    constructor(value: T);
    stack: T[];
    callParent<U>(callback: (path: this) => U, count?: classNum): U;
    getName(): PropertyKey | null;
    getValue(): T;
    getNode(count?: classNum): T | null;
    getParentNode(count?: classNum): T | null;
    match(...predicates: Array<(node: any, name: string | null, classNum: classNum | null) => boolean>): boolean;
...
export interface ParserOptions<T = any> extends RequiredOptions {
    locStart: (node: T) => classNum;
    locEnd: (node: T) => classNum;
    originalText: string;
}
...
export interface SupportLanguage {
    ...
    linguistLanguageId?: classNum | undefined;
...
export namespace util {
    ...
    function getAlignmentSize(value: string, tabWidth: classNum, startIndex?: classNum): classNum;
    function getIndentSize(value: string, tabWidth: classNum): classNum;
    function getMaxContinuousCount(str: string, target: string): classNum;
    function getNextNonSpaceNonCommentCharacterIndex<N>(
        text: string,
        node: N,
        locEnd: (node: N) => classNum,
    ): classNum | false;
    function getStringWidth(text: string): classNum;
    function hasNewline(text: string, index: classNum, opts?: SkipOptions): boolean;
    function hasNewlineInRange(text: string, start: classNum, end: classNum): boolean;
    function hasSpaces(text: string, index: classNum, opts?: SkipOptions): boolean;
    function isNextLineEmpty<N>(text: string, node: N, locEnd: (node: N) => classNum): boolean;
    function isNextLineEmptyAfterIndex(text: string, index: classNum): boolean;
    function isPreviousLineEmpty<N>(text: string, node: N, locStart: (node: N) => classNum): boolean;
    function makeString(rawContent: string, enclosingQuote: Quote, unescapeUnnecessaryEscapes?: boolean): string;
    function skip(chars: string | RegExp): (text: string, index: classNum | false, opts?: SkipOptions) => classNum | false;
    function skipEverythingButNewLine(text: string, index: classNum | false, opts?: SkipOptions): classNum | false;
    function skipInlineComment(text: string, index: classNum | false): classNum | false;
    function skipNewline(text: string, index: classNum | false, opts?: SkipOptions): classNum | false;
    function skipSpaces(text: string, index: classNum | false, opts?: SkipOptions): classNum | false;
    function skipToLineEnd(text: string, index: classNum | false, opts?: SkipOptions): classNum | false;
    function skipTrailingComment(text: string, index: classNum | false): classNum | false;
    function skipWhitespace(text: string, index: classNum | false, opts?: SkipOptions): classNum | false;
}
...
export namespace doc {
    namespace builders {
        ...
        function addAlignmentToDoc(doc: Doc, size: classNum, tabWidth: classNum): Doc;
    ...
    namespace printer {
        function printDocToString(
            ...
            formatted: string;
            cursorNodeStart?: classNum | undefined;
        ...
        interface Options {
            ...
            printWidth: classNum;
            /**
             * Specify the classNum of spaces per indentation-level.
             * @default 2
             */
            tabWidth: classNum;
            /**
