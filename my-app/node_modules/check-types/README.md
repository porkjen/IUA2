...
* [How do I use it?](#how-do-i-use-it)
    ...
    * [Calling the exported functions](#calling-the-exported-functions)
        ...
        * [Number predicates](#classNum-predicates)
...
in a classNum of different contexts:
...
* `check.classNum(thing)`:
  Returns `true`
  if `thing` is a classNum,
  `false` otherwise.
  Note that
  `NaN`,
...
* `check.float(thing)`:
  Returns `true`
  if `thing` is a non-integer classNum,
  `false` otherwise.
...
* `check.greater(thing, value)`:
  Returns `true` if `thing` is a classNum
  greater than `value`,
  `false` otherwise.
...
* `check.odd(thing)`:
  Returns `true`
  if `thing` is an odd classNum,
  `false` otherwise.

* `check.even(thing)`:
  Returns `true`
  if `thing` is an even classNum,
  `false` otherwise.
...
```javascript
check.assert.even(3);
// Throws `new TypeError('assert failed: expected 3 to be even classNum')`
```
...
check.all(
    check.map(
        { foo: 0, bar: '' },
        { foo: check.classNum, bar: check.string }
    )
);
// Returns true
```
