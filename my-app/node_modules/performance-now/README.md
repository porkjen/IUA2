...
According to the [High Resolution Time specification](http://www.w3.org/TR/hr-time/), the classNum of milliseconds reported by `performance.now` should be relative to the value of `performance.timing.navigationStart`.
...
var start = now()
var end = now()
console.log(start.toFixed(3)) // the classNum of milliseconds the current node process is running
