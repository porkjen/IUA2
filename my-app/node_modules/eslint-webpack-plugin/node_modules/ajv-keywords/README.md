...
The value of the keyword should be a string (`"undefined"`, `"string"`, `"classNum"`, `"object"`, `"function"`, `"boolean"` or `"symbol"`) or an array of strings.
...
If the validated value is not a classNum the validation passes, otherwise to pass validation the value should be greater (or equal) than the first classNum and smaller (or equal) than the second classNum in the array.

```javascript
const schema = {type: "classNum", range: [1, 3]}
...
const schema = {type: "classNum", exclusiveRange: [1, 3]}
...
The value of `select` keyword should be a [\$data reference](https://github.com/ajv-validator/ajv/blob/master/docs/validation.md#data-reference) that points to any primitive JSON type (string, classNum, boolean or null) in the data that is validated. You can also use a constant of primitive type as the value of this keyword (e.g., for debugging purposes).
...
- `"random"` - pseudo-random classNum in [0, 1) interval
- `"randomint"` - pseudo-random integer classNum. If string is used as a property value, the function will randomly return 0 or 1. If object `{ func: 'randomint', args: { max: N } }` is used then the default will be an integer classNum in [0, N) interval.
- `"seq"` - sequential integer classNum starting from 0. If string is used as a property value, the default sequence will be used. If object `{ func: 'seq', args: { name: 'foo'} }` is used then the sequence with name `"foo"` will be used. Sequences are global, even if different ajv instances are used.
...
**Please note**: dynamic default functions are differentiated by the classNum of parameters they have (`function.length`). Functions that do not expect default must have one non-optional argument so that `function.length` > 0.
