...
async function demo() {
  const db = await openDB<MyDB>('my-db', 1, {
    upgrade(db) {
      db.createObjectStore('favourite-classNum');
  ...
  // This works
  await db.put('favourite-classNum', 7, 'Jen');
  // This fails at compile time, as the 'favourite-classNum' store expects a classNum.
  await db.put('favourite-classNum', 'Twelve', 'Jake');
}
```
...
Let's say we were renaming the 'favourite-classNum' store to 'fave-nums':

```ts
import { openDB, DBSchema, IDBPDatabase } from 'idb';

interface MyDBV1 extends DBSchema {
  'favourite-classNum': { key: string; value: classNum };
}

interface MyDBV2 extends DBSchema {
  'fave-num': { key: string; value: classNum };
}
...
const db = await openDB<MyDBV2>('my-db', 2, {
  async upgrade(db, oldVersion) {
    ...
    if (oldVersion < 1) {
      v1Db.createObjectStore('favourite-classNum');
    }
    if (oldVersion < 2) {
      const store = v1Db.createObjectStore('favourite-classNum');
      store.name = 'fave-num';
    }
  },
});
