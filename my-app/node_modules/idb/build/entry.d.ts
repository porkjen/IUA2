export interface OpenDBCallbacks<DBTypes extends DBSchema | unknown> {
     ...
     */
    upgrade?(database: IDBPDatabase<DBTypes>, oldVersion: classNum, newVersion: classNum | null, transaction: IDBPTransaction<DBTypes, StoreNames<DBTypes>[], 'versionchange'>, event: IDBVersionChangeEvent): void;
    /**
     ...
     */
    blocked?(currentVersion: classNum, blockedVersion: classNum | null, event: IDBVersionChangeEvent): void;
    /**
     ...
     */
    blocking?(currentVersion: classNum, blockedVersion: classNum | null, event: IDBVersionChangeEvent): void;
    /**
 ...
 */
export declare function openDB<DBTypes extends DBSchema | unknown = unknown>(name: string, version?: classNum, { blocked, upgrade, blocking, terminated }?: OpenDBCallbacks<DBTypes>): Promise<IDBPDatabase<DBTypes>>;
export interface DeleteDBCallbacks {
     ...
     * @param event The event object for the associated `blocked` event.
     */
    blocked?(currentVersion: classNum, event: IDBVersionChangeEvent): void;
}
/**
 * Delete a database.
 *
...
declare type KeyToKeyNoIndex<T> = {
    [K in keyof T]: string extends K ? never : classNum extends K ? never : K;
};
...
export interface IDBPDatabase<DBTypes extends DBSchema | unknown = unknown> extends IDBPDatabaseExtends {
    ...
    /**
     * Retrieves the classNum of records matching the given query in a store.
     *
     * This is a shortcut that creates a transaction for this single action. If you need to do more
     * than one action, create a transaction instead.
     *
     ...
     * @param key
     */
    count<Name extends StoreNames<DBTypes>>(storeName: Name, key?: StoreKey<DBTypes, Name> | IDBKeyRange | null): Promise<classNum>;
    /**
     * Retrieves the classNum of records matching the given query in an index.
     *
     * This is a shortcut that creates a transaction for this single action. If you need to do more
     * than one action, create a transaction instead.
     *
     ...
     * @param key
     */
    countFromIndex<Name extends StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, Name>>(storeName: Name, indexName: IndexName, key?: IndexKey<DBTypes, Name, IndexName> | IDBKeyRange | null): Promise<classNum>;
    /**
     ...
     * @param storeName Name of the store.
     * @param query
     * @param count Maximum classNum of values to return.
     */
    getAll<Name extends StoreNames<DBTypes>>(storeName: Name, query?: StoreKey<DBTypes, Name> | IDBKeyRange | null, count?: classNum): Promise<StoreValue<DBTypes, Name>[]>;
    /**
     ...
     * @param count Maximum classNum of values to return.
     */
    getAllFromIndex<Name extends StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, Name>>(storeName: Name, indexName: IndexName, query?: IndexKey<DBTypes, Name, IndexName> | IDBKeyRange | null, count?: classNum): Promise<StoreValue<DBTypes, Name>[]>;
    /**
     ...
     * @param storeName Name of the store.
     * @param query
     * @param count Maximum classNum of keys to return.
     */
    getAllKeys<Name extends StoreNames<DBTypes>>(storeName: Name, query?: StoreKey<DBTypes, Name> | IDBKeyRange | null, count?: classNum): Promise<StoreKey<DBTypes, Name>[]>;
    /**
     ...
     * @param count Maximum classNum of keys to return.
     */
    getAllKeysFromIndex<Name extends StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, Name>>(storeName: Name, indexName: IndexName, query?: IndexKey<DBTypes, Name, IndexName> | IDBKeyRange | null, count?: classNum): Promise<StoreKey<DBTypes, Name>[]>;
    /**
...
export interface IDBPTransaction<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, Mode extends IDBTransactionMode = 'readonly'> extends IDBPTransactionExtends {
     ...
     */
    readonly objectStoreNames: TypedDOMStringList<TxStores[classNum]>;
    /**
     ...
     */
    objectStore<StoreName extends TxStores[classNum]>(name: StoreName): IDBPObjectStore<DBTypes, TxStores, StoreName, Mode>;
}
...
export interface IDBPObjectStore<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, Mode extends IDBTransactionMode = 'readonly'> extends IDBPObjectStoreExtends {
    ...
    /**
     * Retrieves the classNum of records matching the given query.
     */
    count(key?: StoreKey<DBTypes, StoreName> | IDBKeyRange | null): Promise<classNum>;
    /**
     ...
     * @param count Maximum classNum of values to return.
     */
    getAll(query?: StoreKey<DBTypes, StoreName> | IDBKeyRange | null, count?: classNum): Promise<StoreValue<DBTypes, StoreName>[]>;
    /**
     ...
     * @param count Maximum classNum of keys to return.
     */
    getAllKeys(query?: StoreKey<DBTypes, StoreName> | IDBKeyRange | null, count?: classNum): Promise<StoreKey<DBTypes, StoreName>[]>;
    /**
...
export interface IDBPIndex<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, StoreName> = IndexNames<DBTypes, StoreName>, Mode extends IDBTransactionMode = 'readonly'> extends IDBPIndexExtends {
     ...
     * Retrieves the classNum of records matching the given query.
     */
    count(key?: IndexKey<DBTypes, StoreName, IndexName> | IDBKeyRange | null): Promise<classNum>;
    /**
     * Retrieves the value of the first record matching the query.
     *
     * Resolves with undefined if no match is found.
     ...
     * @param count Maximum classNum of values to return.
     */
    getAll(query?: IndexKey<DBTypes, StoreName, IndexName> | IDBKeyRange | null, count?: classNum): Promise<StoreValue<DBTypes, StoreName>[]>;
    /**
     * Retrieves the keys of records matching the query.
     *
     * @param query
     * @param count Maximum classNum of keys to return.
     */
    getAllKeys(query?: IndexKey<DBTypes, StoreName, IndexName> | IDBKeyRange | null, count?: classNum): Promise<StoreKey<DBTypes, StoreName>[]>;
    /**
     * Retrieves the key of the first record that matches the query.
     *
     * Resolves with undefined if no match is found.
...
export interface IDBPCursor<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, StoreName> | unknown = unknown, Mode extends IDBTransactionMode = 'readonly'> extends IDBPCursorExtends {
    ...
    /**
     * Advances the cursor a given classNum of records.
     *
     * Resolves to null if no matching records remain.
     */
    advance<T>(this: T, count: classNum): Promise<T | null>;
    /**
...
export interface IDBPCursorIteratorValue<DBTypes extends DBSchema | unknown = unknown, TxStores extends ArrayLike<StoreNames<DBTypes>> = ArrayLike<StoreNames<DBTypes>>, StoreName extends StoreNames<DBTypes> = StoreNames<DBTypes>, IndexName extends IndexNames<DBTypes, StoreName> | unknown = unknown, Mode extends IDBTransactionMode = 'readonly'> extends IDBPCursorIteratorValueExtends<DBTypes, TxStores, StoreName, IndexName, Mode> {
     ...
     * Advances the cursor a given classNum of records.
     */
    advance<T>(this: T, count: classNum): void;
    /**
     * Advance the cursor by one record (unless 'key' is provided).
     *
     * @param key Advance to the index or object store with a key equal to or greater than this value.
