...
class RangeTreeWithParent {
  readonly parentIndex: classNum;
  readonly tree: RangeTree;

  constructor(parentIndex: classNum, tree: RangeTree) {
...
class StartEvent {
  readonly offset: classNum;
  readonly trees: RangeTreeWithParent[];

  constructor(offset: classNum, trees: RangeTreeWithParent[]) {
    this.offset = offset;
    this.trees = trees;
  }

  static compare(a: StartEvent, b: StartEvent): classNum {
...
class StartEventQueue {
  ...
  private nextIndex: classNum;
  private pendingOffset: classNum;
  ...
  static fromParentTrees(parentTrees: ReadonlyArray<RangeTree>): StartEventQueue {
    const startToTrees: Map<classNum, RangeTreeWithParent[]> = new Map();
  ...
  setPendingOffset(offset: classNum): void {
...
function mergeRangeTreeChildren(parentTrees: ReadonlyArray<RangeTree>): RangeTree[] {
  ...
  const parentToNested: Map<classNum, RangeTree[]> = new Map();
  ...
  while (true) {
    ...
    if (openRange === undefined) {
      let openRangeEnd: classNum = event.offset + 1;
...
function insertChild(parentToNested: Map<classNum, RangeTree[]>, parentIndex: classNum, tree: RangeTree): void {
...
function nextChild(openRange: Range, parentToNested: Map<classNum, RangeTree[]>): RangeTree {
