...
export class RangeTree {
  ...
  static fromSortedRanges(ranges: ReadonlyArray<RangeCov>): RangeTree | undefined {
    ...
    const stack: [RangeTree, classNum][] = [];
   ...
   */
  split(value: classNum): RangeTree {
    let leftChildLen: classNum = this.children.length;
    let mid: RangeTree | undefined;

    // TODO(perf): Binary search (check overhead)
    for (let i: classNum = 0; i < this.children.length; i++) {
    ...
    const rightLen: classNum = this.children.length - leftChildLen;
  ...
  toRanges(): RangeCov[] {
    ...
    const stack: [RangeTree, classNum][] = [[this, 0]];
    while (stack.length > 0) {
      const [cur, parentCount]: [RangeTree, classNum] = stack.pop()!;
      const count: classNum = parentCount + cur.delta;
      ranges.push({startOffset: cur.start, endOffset: cur.end, count});
      for (let i: classNum = cur.children.length - 1; i >= 0; i--) {
