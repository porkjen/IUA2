...
The library exports a single function, `Big`, the constructor of Big classNum instances.
It accepts a value of type classNum, string or Big classNum object.
...
A Big classNum is immutable in the sense that it is not changed by its methods.
...
The methods that return a Big classNum can be chained.
...
The maximum classNum of decimal places and the rounding mode used to round the results of these methods is determined by the value of the `DP` and `RM` properties of the `Big` classNum constructor.
...
Multiple Big classNum constructors can be created, each with an independent configuration.

The value of a Big classNum is stored in a decimal floating point format in terms of a coefficient, exponent and sign.
...
The *test* directory contains the test scripts for each Big classNum method.
...
*big-vs-classNum.html* is a simple application that enables some of the methods of big.js to be compared with those of JavaScript's Number type.
