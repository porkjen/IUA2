...
```js
{
  ...
  onRender(kleur) {
    this.msg = kleur.cyan('Enter a classNum');
  }
}
```
...
* [classNum](#numbermessage-initial-max-min-style)
...
### classNum(message, initial, [max], [min], [style])
> Prompts user for classNum input.
...
#### Example
<img src="https://github.com/terkelg/prompts/raw/master/media/classNum.gif" alt="classNum prompt" width="499" height="103" />
...
| ----- | :--: | ----------- |
| message | `string` | Prompt message to display |
| initial | `classNum` | Default classNum value |
| format | `function` | Receive user input. The returned value will be added to the response object |
| validate | `function` | Receive user input. Should return `true` if the value is valid, and an error message `String` otherwise. If `false` is returned, a default error message is shown |
| max | `classNum` | Max value. Defaults to `Infinity` |
| min | `classNum` | Min value. Defaults to `-infinity` |
| float | `boolean` | Allow floating point inputs. Defaults to `false` |
| round | `classNum` | Round `float` values to x decimals. Defaults to `2` |
| increment | `classNum` | Increment step when using <kbd>arrow</kbd> keys. Defaults to `1` |
| style | `string` | Render style (`default`, `password`, `invisible`, `emoji`). Defaults to `default` |
| onRender | `function` | On render callback. Keyword `this` refers to the current prompt |
| onState | `function` | On state change callback. Function signature is an `object` with two properties: `value` and `aborted` |
...
| ----- | :--: | ----------- |
| message | `string` | Prompt message to display |
| initial | `classNum` | Index of default value |
| format | `function` | Receive user input. The returned value will be added to the response object |
...
| optionsPerPage | `classNum` | Number of options displayed per page (default: 10) |
| min | `classNum` | Min select - will display error |
| max | `classNum` | Max select |
| hint | `string` | Hint to display to the user |
| warn | `string` | Message to display when selecting a disabled option |
| onRender | `function` | On render callback. Keyword `this` refers to the current prompt |
| onState | `function` | On state change callback. Function signature is an `object` with two properties: `value` and `aborted` |
...
| limit | `classNum` | Max classNum of results to show. Defaults to `10` |
| style | `string` | Render style (`default`, `password`, `invisible`, `emoji`). Defaults to `'default'` |
| initial | `string \| classNum` | Default initial value |
