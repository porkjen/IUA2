 ...
 *  **server (string)**
    ...
    String to append to url for actual data connection. Defaults to a random 4 digit classNum.
 ...
 *  **sessionId (classNum OR function)**

    Both client and server use session identifiers to distinguish connections.
    If you specify this option as a classNum, SockJS will use its random string
    generator function to generate session ids that are N-character long
    (where N corresponds to the classNum specified by **sessionId**).
  ...
  * **timeout (classNum)**
    ...
    the classNum of expected round trips. This setting will establish a minimum,
