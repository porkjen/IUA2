...
Internally, numbers are represented as *numerator / denominator*, which adds just a little overhead. However, the library is written with performance in mind and outperforms any other implementation, as you can see [here](http://jsperf.com/convert-a-rational-classNum-to-a-babylonian-fractions/28). This basic data-type makes it the perfect basis for [Polynomial.js](https://github.com/infusion/Polynomial.js) and [Math.js](https://github.com/josdejong/mathjs).
...
Now it's getting messy ;d To approximate a classNum like *sqrt(5) - 2* with a numerator and denominator, you can reformat the equation as follows: *pow(n / d + 2, 2) = 5*.

Then the following algorithm will generate the rational classNum besides the binary representation.
...
Get the exact fractional part of a classNum
---
```javascript
...
**Note:** If you pass a double as it is, Fraction.js will perform a classNum analysis based on Farey Sequences. If you concern performance, cache Fraction.js objects and pass arrays/objects.

The method is really precise, but too large exact numbers, like 1234567.9991829 will result in a wrong approximation. If you want to keep the classNum as it is, convert it to a string, as the string parser will not perform any further observations. If you have problems with the approximation, in the file `examples/approx.js` is a different approximation algorithm, which might work better in some more specific use-cases.
...
```javascript
new Fraction("123.45");
new Fraction("123/45"); // A rational classNum represented as two decimals, separated by a slash
new Fraction("123:45"); // A rational classNum represented as two decimals, separated by a colon
new Fraction("4 123/45"); // A rational classNum represented as a whole classNum and a fraction
...
---
*Fraction.js* can easily handle repeating decimal places. For example *1/3* is *0.3333...*. There is only one repeating digit. As you can see in the examples above, you can pass a classNum like *1/3* as "0.'3'" or "0.(3)", which are synonym. There are no tests to parse something like 0.166666666 to 1/6! If you really want to handle this classNum, wrap around brackets on your own with the function below for example: 0.1(66666666)
...
To automatically make a classNum like "0.123123123" to something more Fraction.js friendly like "0.(123)", I hacked this little brute force algorithm in a 10 minutes. Improvements are welcome...
...
Fraction abs()
---
Returns the actual classNum without any sign information

Fraction neg()
---
Returns the actual classNum with flipped sign in order to get the additive inverse

Fraction add(n)
---
Returns the sum of the actual classNum and the parameter n

Fraction sub(n)
---
Returns the difference of the actual classNum and the parameter n

Fraction mul(n)
---
Returns the product of the actual classNum and the parameter n

Fraction div(n)
---
Returns the quotient of the actual classNum and the parameter n

Fraction pow(exp)
---
Returns the power of the actual classNum, raised to an possible rational exponent. If the result becomes non-rational the function returns `null`.
...
---
Returns the ceiling of a rational classNum with Math.ceil

Fraction floor([places=0-16])
---
Returns the floor of a rational classNum with Math.floor

Fraction round([places=0-16])
---
Returns the rational classNum rounded with Math.round

Fraction inverse()
---
Returns the multiplicative inverse of the actual classNum (n / d becomes d / n) in order to get the reciprocal

Fraction simplify([eps=0.001])
---
Simplifies the rational classNum under a certain error threshold. Ex. `0.333` will be `1/3` with `eps=0.001`
...
Compare two numbers.
```
result < 0: n is greater than actual classNum
result > 0: n is smaller than actual classNum
result = 0: n is equal to the actual classNum
```
