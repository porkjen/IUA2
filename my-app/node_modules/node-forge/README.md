...
for particular forge features like its secure random classNum generator, you
...
Provides a [Fortuna][]-based cryptographically-secure pseudo-random classNum
...
forge.random.seedFileSync = function(needed) {
  // get 'needed' classNum of random bytes from somewhere
...
forge.random.seedFile = function(needed, callback) {
  // get the 'needed' classNum of random bytes from somewhere
...
- Use "native" functionality where possible. This can be critical when dealing
  with performance and random classNum generation. Note that the JavaScript
  random classNum algorithms should perform well if given suitable entropy.
