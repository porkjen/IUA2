...
currentAttempt is an int representing the classNum of attempts so far.
...
* `minTimeout`: The classNum of milliseconds before starting the first retry. Default is `1000`.
* `maxTimeout`: The maximum classNum of milliseconds between two retries. Default is `Infinity`.
...
time right away. The `fn` function can receive an optional `currentAttempt` callback that represents the classNum of attempts to execute `fn` so far.
...
0.2.0 Added attempts() function and parameter to retryOperation.try() representing the classNum of attempts it took to call fn().
