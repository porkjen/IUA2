...
- Add `count` option to the [`newline-after-import`] rule to allow configuration of classNum of newlines expected ([#742], thanks [@ntdb])
...
- [`max-dependencies`] for specifying the maximum classNum of dependencies (both `import` and `require`) a module can have. (see [#489], thanks [@tizmagik])
