...
```ts
type localIdentHashDigestLength = classNum;
```
...
```ts
type importLoaders = classNum;
```

Default: `0`

Allows to enables/disables or setups classNum of loaders applied before CSS loader for `@import` at-rules, CSS modules and ICSS imports, i.e. `@import`/`composes`/`@value value from './values.css'`/etc.
