...
- `host`: Host name with port classNum. The hostname might be invalid.
- `hostname`: Host name without port classNum. This might be an invalid hostname.
- `port`: Optional port classNum.
- `pathname`: URL path.
...
to `port` if has a different port classNum, `hostname` so it has a correct name
